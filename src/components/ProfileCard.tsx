...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  artistImage = '<Placeholder for icon URL>',
  ...
  const animationHandlers = useMemo(() => {
    ...
    const updateCardTransform = (offsetX: number, offsetY: number, card: HTMLElement, wrap: HTMLElement) => {
      ...
      const percentX = clamp((20 / width) * offsetX);
      const percentY = clamp((20 / height) * offsetY);

      const centerX = percentX - 0;
      const centerY = percentY - 0;
  ...
  const cardStyle = useMemo(
    () =>
      ({
        '--icon': artistImage ? `url(${artistImage})` : 'none',
        '--grain': grainUrl ? `url(${grainUrl})` : 'none',
        '--behind-gradient': showBehindGradient ? (behindGradient ?? DEFAULT_BEHIND_GRADIENT) : 'none',
        '--inner-gradient': innerGradient ?? DEFAULT_INNER_GRADIENT
      }) as React.CSSProperties,
    [artistImage, grainUrl, showBehindGradient, behindGradient, innerGradient]
  );
    ...
    <div ref={wrapRef} className={`pc-card-wrapper ${className}`.trim()} style={cardStyle}>
      <section ref={cardRef} className="pc-card">
        <div className="pc-inside">
            ...
            /> 
          <div className="pc-content pc-cover-content">
          </div>
        </div>
      </section>
    </div>
    <div className='pc-user-info-container'>
                  <div className="pc-user-info">
                <div className="pc-user-details">
                  <div className="pc-artist-image">
                    <img
                      src={artistImage}
                      alt={`${artist || 'User'} mini cover`}
                      loading="lazy"
                      onError={e => {
                        const target = e.target as HTMLImageElement;
                  ...
                  <div className="pc-user-text">
                    <div className=".pc-title-artist">{title}</div>
                    <div className="pc-artist">{artist}</div>
                  </div>
                </div>
              </div>
            </div>
